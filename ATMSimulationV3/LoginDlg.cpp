...
void LoginDlg::on_LoginPB_clicked()
{
    ...
    if(user == "" || passw == "") {
        QMessageBox::information(this, tr("warning!"), tr("Please enter the correct user name and password!"), QMessageBox::Yes);//请输入正确的用户名和密码！
    }
    else
    {
        ...
        {   // 若能查找到输入的用户名，则要判断密码是否正确
            ...
            if(user.passWord == passw)
            {
                ...
                QMessageBox::information(this, tr("warning"), tr("Please enter the correct user name and password!"));//请输入正确的用户名和密码！
            }
        }
        else {
            QMessageBox::information(this, tr("warning"), tr("The user is not found, please re-enter!"));//未找到该用户，请重新输入！
        }
    }
}
