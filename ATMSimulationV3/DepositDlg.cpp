...
void DepositDlg::on_pushButton_ok_clicked()
{
    ...
    if(rmb > 0 && rmb % 100 == 0)
    {
        if(m_Data.changeBalance(rmb)) {
            QMessageBox::about(NULL, tr("message!"), tr("Deposit successful!"));//存款成功！
            ui->lineEdit_RMB->clear();
        }
        else {
            QMessageBox::critical(NULL, tr("message!"), tr("Deposit failed!"));//存款失败！
        }
    }
    else {
        QMessageBox::warning(NULL, tr("message!"), tr("an integral multiple of 100!"));//存款金额必须为100的整数倍！
    }
}
