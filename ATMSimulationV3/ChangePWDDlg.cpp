...
void ChangePWDDlg::on_pushButton_ok_clicked()
{
    ...
    if(newPWD == newPWD_2 && newPWD.size() >= 6 && maxContinue(newPWD) < 6)
    {
        ...
        if(newPWD != newPWD_2) {
            QMessageBox::warning(NULL, tr("warning!"), tr("The two passwords must be the same!"));//两次输入的密码必须一致！
        }
        else if(newPWD.size() < 6){
            QMessageBox::warning(NULL, tr("warning!"), tr("Password must be greater than 6 digits!"));//密码必须大于六位！
        }
        else {
            QMessageBox::warning(NULL, tr("warning!"), tr("The password cannot appear 6 consecutive digits the same!"));//密码不能出现连续6位相同！
        }
    }
}
