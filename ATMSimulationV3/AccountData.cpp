...
bool AccountData::changeBalance(int rmb)
{
    ...
    {
        QMessageBox::warning(NULL, "warning!", "Failed, insufficient balance!");//失败，余额不足！
        return false;
    }
}
...
bool AccountData::changePWD(QString oldPWD, QString newPWD)
{
    ...
    if(user.passWord == oldPWD)
    {
        ...
        save(); // 将更新后的信息写入到文件
        QMessageBox::about(NULL, "message!", "Password changed successfully!");//密码更改成功！
        return true;
    }
    else
    {
        QMessageBox::critical(NULL, "Old password error!", "Please enter the correct old password!");//请输入正确的旧密码！
        return false;
    }
}
        ...
        iter != m_accounts.end(); iter++)
    {
        ...
        if(user.account == aim)     // 如果找到了目标账户
        {
            if(changeBalance(-rmb)) // 先在自己账户上扣除相应金额，扣除成功再转账
            {
                ...
                save();                 // 更新文件信息
                QMessageBox::about(NULL, "message!", "Transfer succeeded!");//转账成功！
                return true;
            }
        }
    }
    QMessageBox::question(NULL, "message!", "No target account, transfer failed!");//没有目标账户，转账失败！
    return false;
}
